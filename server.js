/*
  All tbe server logic and live auction logic goes here
  */
...
io.on("connect", (socket) => {
  ...
  /*
    This function is called when the auction timer is over
     */
  socket.on("sold", (data) => {
    console.log(data);
    console.log("yoo");
    ...
    Openbid.findOne({
      ...
      console.log(bid);
      if (bid != null) {
        await Bidding.find({ bidid: data.bidid })
          .sort({ amount: -1 })
          .limit(1)
          .then((allbids) => {
            if (allbids.length > 0) {
  ...
  /*
   This function is called when ever a user bids
    */
  ...
  socket.on("typing", (data) => {
    socket.broadcast
      .in(data.room)
      .emit("typing", { username: socket.username });
  });
  ...
  socket.on("new_auction_chat_message", (data) => {
    ...
    socket.broadcast
      .in(data.room)
      .emit("auction_chat", { message: data.message });
  });
  ...
  socket.on("new_exchange_chat_message", (data) => {
    ...
    socket.broadcast
      .in(data.room)
      .emit("exchange_chat", { message: data.message });
  });
});

/*
  Scheduled task to remove auctions in which no one participated
   */
const staleAuctionRemoveJob = new CronJob(
  "0 */10 * * * *",
  function () {
    const d = new Date();
...
/*
   Scheduled task to remove books for which borrow duration is over
   */
const expiredBooksRemoveJob = new CronJob(
  "0 */10 * * * *",
  function () {
    Shelf.deleteMany({ period: { $exists: true, $lt: new Date() } }).then(
      async (x) => {
...
const similarBooksJob = new CronJob(
  "5 1 * * Sun",
  function () {
