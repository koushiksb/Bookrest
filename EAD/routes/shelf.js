...
router.get('/viewbook/:title', isLoggedIn.isLoggedIn, (req, res) => {
  ...
  Book.findOne({ Title: req.params.title }).lean().then(async (x) => {
    ...
    res.render('viewbook', { desc:x.Description,image: x.ImageURLL,type:x.Class, title: x.Title, otherUserShelf: false, author: x.Author, inbidding: inbidding, id: x._id, owner: owner, softCopy: softCopy, readRequestAmount: readRequestAmount, hasHardCopy: hasHardCopy, layout: 'navbar2.ejs' });
...
router.get('/otherUserShelfviewbook/:title/:userid', (req, res) => {
  ...
  Book.findOne({ Title: req.params.title }).lean().then(async (x) => {
    ...
    res.render('viewbook', {desc:x.Description,type:x.Class, image: x.ImageURLL, readRequestAmount: readRequestAmount, title: x.Title, hasHardCopy: false, otherUserShelf: true, author: x.Author, inbidding: inbidding, id: x._id, owner: owner, softCopy: softCopy, layout: 'navbar2.ejs' });
...
router.get('/viewbk/:title', async (req, res) => {
  ...
  Book.findOne({ Title: req.params.title.toString() }).populate({ path: 'Similar', model: 'Book' }).lean().then(async (x) => {
    ...
    Review.find({ book: x._id }).populate({ path: 'user', model: 'User', populate: { path: 'profile', model: 'Profile' } }).then(async (y) => {
      ...
      console.log(x)
      res.render('viewbk', {desc:x.Description,type:x.Class, similar: x.Similar, suggest: suggest, given: given, image: x.ImageURLL, genre: x.Genre, rating: (x.Rating / 2).toFixed(1), title: x.Title, author: x.Author, reviews: y, col1: col1, col2: col2, otherUsers: otherUsers, layout: navbar });
