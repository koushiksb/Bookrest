...
router.get("/confirm/:bookReq/:bookSen", (req, res) => {
  Book.find({ Title: req.params.bookReq }).then((x) => {
    Book.find({ Title: req.params.bookSen }).then((y) => {
      ...
      // console.log(y)
      return res.render("confirmexchange", {
        bookSen: y[0],
        bookReq: x[0],
        userReq: userReq,
      });
...
router.get("/require/:title", isLoggedIn.isLoggedIn, (req, res) => {
  Book.findOne({ Title: req.params.title }).then((x) => {
    userReq = req.user._id;
    Shelf.find({ user: req.user._id, owner: { $exists: false } })
      .select("book -_id")
...
router.get("/mytrades", isLoggedIn.isLoggedIn, (req, res) => {
  Exchange.find({ userReq: req.user.id, status: false })
    .populate({
      path: "userReq",
      model: "User",
    ...
    .populate("bookReq bookSen", "Title ImageURLL Author")
    .then((x) => {
    ...
    .then((x) => {
      ...
      Exchange.find({
        ...
        .then((y) => {
          ...
          return res.render("tradescompleted", {
            requests: x,
            accepts: y,
            layout: "navbar2",
          });
...
router.get("/ongoing", isLoggedIn.isLoggedIn, (req, res) => {
  Exchange.find({
    userReq: req.user.id,
    status: true,
    $or: [{ exchangeReq: false }, { exchangeSen: false }],
  })
    .populate({
      path: "userReq",
      model: "User",
    ...
    .then((x) => {
      ...
      if (x.userReq._id == req.user.id) {
        ...
        var n = new Notify({
          ...
          Type: "Book Received",
          Message:
            x.userReq.profile.fname +
            " " +
            x.userReq.profile.lname +
            " has received the book you sent.",
        }).save();
      } else {
        x.exchangeSen = true;
        var n = new Notify({
          ...
          Type: "Book Received",
          Message:
            x.userAcc.profile.fname +
            " " +
            x.userAcc.profile.lname +
            " has received the book you sent.",
        }).save();
        console.log("hhknjn");
      }
        ...
        .then((a) => {
          ...
          if (a.exchangeReq && a.exchangeSen) {
            ...
            var n = new Notify({
              ...
              Message:
                "Exchange with " +
                a.userReq.profile.fname +
                " " +
                a.userReq.profile.lname +
                " has been done. Bookshelf is updated.",
            }).save();
            var n = new Notify({
              User: a.userReq,
              Type: "Book Exchanged",
              Message:
                "Exchange with " +
                a.userAcc.profile.fname +
                " " +
                a.userAcc.profile.lname +
                " has been done. Bookshelf is updated.",
            }).save();
...
router.get("/tradepage", (req, res) => {
  ...
  Exchange.find({ status: false })
    .populate({
      path: "userReq",
      model: "User",
      populate: { path: "profile", model: "Profile" },
    })
    .populate("bookReq bookSen", "Title ImageURLL")
    .then((x) => {
      var gen = [];
...
router.post("/filtertradepage", (req, res) => {
  ...
  Exchange.find({ status: false })
    ...
    .populate("bookReq bookSen", "Title ImageURLL Genre")
    .then((x) => {
      var gen = [];
      console.log(x);
      for (var i = 0; i < x.length; i++) {
        ...
        if (location.includes(x[i].userReq.profile.location.toLowerCase())) {
          if (
            genreFilters.includes(x[i].bookReq.Genre) ||
            genreFilters.includes(x[i].bookSen.Genre)
          ) {
            if (
              x[i].userReq.id != req.user._id &&
              books.includes(x[i].bookReq.id)
            ) {
              gen.push(x[i]);
            }
          }
...
router.get("/accept/:id", isLoggedIn.isLoggedIn, (req, res) => {
  Exchange.findOneAndUpdate(
    { _id: req.params.id },
    { status: true, userAcc: req.user.id }
  ).then(async (x) => {
    console.log(x);
    await Exchange.findOne({ _id: req.params.id })
      .populate({
        path: "userAcc",
        model: "User",
        populate: { path: "profile", model: "Profile" },
      ...
      .then(async (y) => {
        var n = new Notify({
          ...
          Message:
            y.userAcc.profile.fname +
            " " +
            y.userAcc.profile.lname +
            " has accepted to exchange the book that you offered.",
        }).save();
