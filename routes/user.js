...
router.get("/signup", IsAuth, (req, res) =>
  res.render("signup", { errors: [], check: 0 })
);
/*
...
router.post(
  ...
  [
    ...
    check("password")
      ...
      .withMessage(
        "Passwordmust contain one uppercase letter one lower case letter and one special character  "
      ),
    check("password1")
      .not()
      .equals("password")
...
router.get("/notify", isLoggedIn, (req, res) => {
  ...
  var types = {
    ...
    "Book Received": 0,
    "Book Exchanged": 1,
    "View Read Requests": 2,
    "Read Request Status": 3,
  };
  var urlseen = [];
  var urlunseen = [];
  Notify.find({ User: req.user.id })
    .sort({ date: 1 })
    .then((y) => {
      console.log(y);
  ...
  if (req.user) {
    if (req.user.profile == undefined) {
      ...
      User.findOne({ _id: req.user.id })
        .populate({ path: "Recommend", model: "Book" })
        .lean()
        .then((g) => {
          if (g.Recommend) {
      ...
      Book.find({ Class: "Rare" })
        .limit(8)
        .lean()
        .then((f) => {
          pop["Rare"] = f;
        });
      Book.find({ Class: "Popular" })
        .limit(8)
        .lean()
        .then((f) => {
          pop["Popular"] = f;
...
router.post(
  "/profile",
  multer(userimageconf).single("profilepic"),
  (req, res) => {
    ...
    if (req.user.profile == undefined) {
      ...
      Profile.findById({ _id: req.user.profile }).then((x) => {
        ...
        x.phone = phone;
        x.address = address;
        (x.favouriteGenre = favouriteGenre), (x.profilepic = propic);
        x.save().then((z) => {
  ...
  Token.findOne({ token: req.params.token }, function (err, token) {
    ...
    User.findOne({ _id: token._userId }, function (err, user) {
      ...
      user.save(function (err) {
        if (err) {
          return res.render("msg", { msg: err.message, msg1: "Not Verified" });
        }
        return res.render("msg", {
