...
router.get("/viewrequest", isLoggedIn.isLoggedIn, (req, res) => {
  console.log("found");
  RareRequest.find({ recipient: req.user.id, status: 0 })
    .populate({
      path: "requester",
      model: "User",
      populate: { path: "profile", model: "Profile" },
    })
    .populate("book", "Title ImageURLL Author")
    .then((x) => {
...
router.get("/viewsentrequest", (req, res) => {
  console.log("found");
  RareRequest.find({ requester: req.user.id, status: 0 })
    .populate({
      path: "recipient",
      model: "User",
      populate: { path: "profile", model: "Profile" },
...
router.get("/oldrequests", isLoggedIn.isLoggedIn, (req, res) => {
  RareRequest.find({ requester: req.user.id, status: { $ne: 0 } })
    .populate({
      path: "recipient",
      model: "User",
      populate: { path: "profile", model: "Profile" },
...
router.post("/deleterequest", isLoggedIn.isLoggedIn, (req, res) => {
  RareRequest.findOneAndDelete({ _id: req.body.id })
    .then((x) => {
      res.sendStatus(200);
    })
    .catch((err) => {
...
router.post("/viewrequest", isLoggedIn.isLoggedIn, (req, res) => {
  ...
  console.log(req.body);
  var noOfDays = 0;
  RareRequest.find({ book: req.body.bookid, recipient: req.user._id })
    .then(async (x) => {
      x = x[x.length - 1];
      console.log(x);
      ...
      var softcopylink = "";
      var amoutPerDay = 5;
      await Shelf.findOne({ user: req.user.id, book: req.body.bookid }).then(
        (softCopy) => {
